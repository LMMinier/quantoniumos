...
\title{Resonance Fourier Transform:\\
...
The Resonance Fourier Transform (RFT) was introduced and formally analyzed in~\cite{minier2025rft} (DOI: 10.5281/zenodo.17712905) as a unitary family of transforms built on irrational, golden-ratio--based phase progressions and braided toroidal topologies, with a canonical Gram-normalized $\Phi$-RFT for theory and a closed-form fast $\Phi$-RFT for practice. This paper extends that foundational work by demonstrating RFT's practical application in hybrid compression, cryptographic mixing, and quantum-inspired computing. On top of this basis, we build: (i) a rigorously validated stack of $\Phi$-RFT theorems covering unitarity, complexity, sparsity, and twisted-convolution algebra; (ii) a hybrid DCT+RFT codec that beats single-basis baselines on mixed-structure signals; (iii) an RFT-SIS experimental lattice playground with measured avalanche/diffusion; and (iv) a unified C/FPGA execution path that removes Python from the hot loop and demonstrates that this resonance-based approach is not just mathematically consistent, but also system-level executable. 
...
\begin{itemize}
  ...
  \item \textbf{Canonical vs.\ Fast forms.} The canonical $\Phi$-RFT is the Gram-normalized $\phi$-grid exponential basis $\widetilde{\Phi} = \Phi(\Phi^H\Phi)^{-1/2}$, used to prove sparsity, non-equivalence to LCT/FrFT, and quantum-chaos-style level statistics. The closed-form fast $\Phi$-RFT is an $O(N\log N)$ factorization $\Psi = D_\phi C_\sigma F$ used in real compute paths, with unitarity and twisted-convolution algebra verified numerically. A legacy QR construction over a resonance matrix $R$ is retained for historical comparison but is not the canonical definition.
...
\midrule
Transform speed & FFT ecosystem (NumPy/SciPy/FFTW/MKL) & $\Phi$-RFT is 1.3--3.9$\times$ slower; same $O(n\log n)$ scaling but higher constant factors. \\
Compression & gzip, LZMA, Zstd, Brotli, LZ4 & RFTMW ratios 1.95--2.83$\times$ on tested datasets; industrial codecs achieve 100--600$\times$ on the same data. \\
...
\end{bmatrix},
\]
which is generally full-rank but neither orthogonal nor normalized. In this repository, the canonical $\Phi$-RFT is instead defined as the Gram-normalized $\phi$-grid exponential basis $\widetilde{\Phi} = \Phi(\Phi^H\Phi)^{-1/2}$. The QR-orthonormalized $R$ basis is retained as a legacy/alternative construction, while the fast $\Phi$-RFT enforces golden-phase structure via an explicit factorization built on top of the FFT.

\subsection{Canonical $\Phi$-RFT (Gram-Normalized, $O(N^3)$)}
The \emph{canonical} $\Phi$-RFT is constructed by Gram-matrix normalization (Loewdin orthogonalization) of the raw $\phi$-grid exponential basis $\Phi$. Let
\[
\widetilde{\Phi} = \Phi(\Phi^H\Phi)^{-1/2},
\]
and define the canonical $\Phi$-RFT and its inverse as
\[
\widehat{x} \;=\; \widetilde{\Phi}^\dagger x,
\qquad
x \;=\; \widetilde{\Phi} \widehat{x}.
\]

Because $\widetilde{\Phi}$ is exactly unitary by construction, this transform preserves $\ell_2$ energy and inner products to machine precision. The cost, however, is cubic: the Gram normalization scales as $O(N^3)$, making it a mathematical gold standard, not a production kernel.
...
\begin{itemize}
  ...
  = F^\dagger F = I.
  \]
  Numerically, the implementation in \path{algorithms/rft/core/phi_phase_fft_optimized.py} satisfies $\max_{i,j} \big|(\Psi^\dagger \Psi - I)_{ij}\big| \lesssim 10^{-14}$ for $N \leq 512$.
...
\begin{itemize}
  ...
  \item The canonical $\Phi$-RFT is the Gram-normalized $\phi$-grid exponential basis $\widetilde{\Phi} = \Phi(\Phi^H\Phi)^{-1/2}$.
  \item The fast $\Phi$-RFT, $\Psi = D_\phi C_\sigma F$, is engineered to be unitary by construction, FFT-class, and expressible as a composition of simple unitary factors. A QR/MGS resonance-matrix basis is retained as a legacy/alternative reference only.
\end{itemize}
...
\begin{itemize}
  \item sparsity and chaos results (Theorems 3--6) are formally proven for the canonical basis $\widetilde{\Phi}$;
...
In practice, floating-point error introduces small deviations. For the current \path{algorithms/rft/core/phi_phase_fft_optimized.py} implementation, a representative slice is:
...
\begin{itemize}
  \item \path{algorithms/rft/core/phi_phase_fft_optimized.py} (fast $\Phi$-RFT),
...
Comprehensive validation of the seven RFT variants (Standard, Harmonic, Fibonacci, Chaotic, Geometric, Hybrid, Adaptive) was performed using the canonical Gram-normalized construction (with legacy QR runs retained for comparison). Table~\ref{tab:unitarity_validation} summarizes unitarity errors across transform sizes.
...
To validate the mathematical correctness of the RFT variants at a low level, we implemented a high-performance C/Assembly backend (\path{libquantum_symbolic.so}). This backend implements a \emph{legacy QR/MGS reference} basis to ensure numerical stability and exact unitarity for comparison against the canonical Gram-normalized definition.
...
\begin{itemize}
    ...
    \item \textbf{Sparsity:} The legacy MGS construction in C yields lower sparsity than the Fast $\Phi$-RFT for certain quasi-periodic signals, highlighting the structural difference between the legacy QR basis and the canonical/fast constructions.
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item \path{algorithms/rft/core/phi_phase_fft_optimized.py} --- fast $\Phi$-RFT ($\Psi = D_\phi C_\sigma F$). \emph{Stable}.
    \item \path{algorithms/rft/core/resonant_fourier_transform.py} --- canonical Gram-normalized $\Phi$-RFT. \emph{Stable}.
    \item \path{algorithms/rft/core/canonical_true_rft.py} --- legacy QR reference basis. \emph{Legacy}.
  \end{itemize}
...
\begin{itemize}
  \item \textbf{Canonical $\Phi$-RFT} ($\widetilde{\Phi}$): Gram-normalized, $O(N^3)$, home of sparsity, chaos, and non-equivalence theorems.
...
\section{Conclusion}
This work presents $\Phi$-RFT as a lens for regimes misaligned with classical integer-harmonic bases. We separated canonical $\Phi$-RFT (Gram-normalized, $O(N^3)$) from fast $\Phi$-RFT ($\Psi = D_\phi C_\sigma F$, $O(N \log N)$), deployed a C/FPGA stack, and validated end-to-end.
