...
This flow:
- Generates Q1.15 `$readmemh` vectors for the golden-drift ensemble.
- Generates a Q1.15 complex kernel for the canonical basis (conj(U)).
- Runs a SystemVerilog/Verilator simulation that dumps per-case coefficient energies.
- Runs a host-side Python verifier that computes K99(RFT), K99(FFT), and a bootstrap CI over the paired improvement.

2) **Legacy (archival): hex vectors + standalone testbenches.**

```bash
cd hardware
python theorem8_concentration_test.py
```

This produces archived artifacts under `hardware_test_vectors/theorem8/` (hex vectors, legacy SV testbenches, JSON summaries, LaTeX table).

### Empirical Results (500 samples per N)

| N | E[K₀.₉₉(RFT)] | σ | E[K₀.₉₉(FFT)] | σ | Gap | Gap (%) |
|---|---------------|---|---------------|---|-----|---------|
...
**Result:** The inequality holds for all tested sizes (p < 0.001 at N≥64).

Note: the table above is from the legacy (archival) generator path. The recommended Option A pipeline reports its own bootstrap CI and a boolean pass/fail (`theorem_holds`).

### Hardware Verification Files

| File | Purpose |
|------|---------|
| `hardware/tb/Makefile` | End-to-end `theorem8-run` target (Option A) |
| `hardware/tb/tb_theorem8_energy_dump.sv` | SV testbench: computes transform outputs and dumps energies |
| `hardware/tb/theorem8_vectors_q15.py` | Generates Q1.15 golden-drift vectors for `$readmemh` |
| `hardware/tb/theorem8_kernel_uconj_q15.py` | Generates Q1.15 complex conj(U) kernel for canonical basis |
| `hardware/tb/verify_theorem8_option_a.py` | Host verifier: K99 + bootstrap CI + pass/fail |
| `hardware/theorem8_concentration_test.py` | Legacy test vector generator and benchmark (archival) |
| `hardware_test_vectors/theorem8/theorem8_vectors_N*.hex` | Legacy Q16.16 fixed-point test data (archival) |
| `hardware_test_vectors/theorem8/theorem8_tb_N*.sv` | Legacy SystemVerilog testbenches (archival) |
| `hardware_test_vectors/theorem8/theorem8_summary_N*.json` | Legacy expected results (archival) |
| `hardware_test_vectors/theorem8/theorem8_latex_table.tex` | Legacy paper-ready table (archival) |

### Integration with RFTPU

To verify Theorem 8 on the RFTPU:

1. **Option A (recommended)**: implement transform + energy dump in RTL, and compute K99 on the host.
   - Hardware/RTL computes transform coefficients (or energies) for both RFT and FFT paths.
   - Hardware/RTL exports per-case coefficient energies (or magnitudes) over a debug channel.
   - Host computes K99 and runs bootstrap CI for the paired improvement.
   - Acceptance criterion: bootstrap CI excludes 0 and the run reports `theorem_holds=True`.

2. **Test Flow**:
   - Load golden quasi-periodic signals (from `hardware/tb` memh vectors or legacy hex vectors)
   - Compute both RFT and FFT transforms
   - Export coefficients/energies
   - Compute K99 and statistical pass/fail on the host

3. **Acceptance Criterion**: E[K99(RFT)] < E[K99(FFT)] over the test suite.

### CI Test Hook

The theorem is also verified in Python CI:

```bash
pytest tests/proofs/test_rft_transform_theorems.py -k theorem_8 -v
```

All 4 Theorem 8 tests pass (concentration inequality, negative control,
scaling, random baseline).

---

## 15. Wave-Space Computing Extensions (Software Prototype)

The repo includes a **software wave-space compute prototype** that demonstrates
what computations can be done directly on complex coefficient vectors (no bit
decoding). This is distinct from (and currently ahead of) the RTL feature set.

**Implementation:**
- Wave-space engine: [algorithms/rft/core/true_wave_compute.py](../algorithms/rft/core/true_wave_compute.py)

**Wave-native primitives (coeff-space):**
- Superposition (linear combination)
- Filtering (masking)
- Spectral multiply (convolution theorem)
- Phase shift / amplitude scaling
- Composition helpers: coefficient shift, magnitude-only nonlinearity
- Soft conditional selection (DC-energy interpolation)

**Heuristic soft logic:**

Masking-based AND/OR/NOT helpers exist for experimentation, but they are not
true bitwise gates (they depend on magnitude thresholding).

**Validation:**
- Sparse convolution theorem sanity: [tests/proofs/test_sparse_convolution_speedup.py](../tests/proofs/test_sparse_convolution_speedup.py)
- Soft logic / soft conditional: [tests/proofs/test_wave_space_soft_logic.py](../tests/proofs/test_wave_space_soft_logic.py)

Run:

```bash
python -m pytest -q tests/proofs/test_sparse_convolution_speedup.py \
   tests/proofs/test_wave_space_soft_logic.py
```

**Hardware note:** This section documents *software* behavior and testable
mathematical identities. RTL equivalents (e.g., mask units, spectral multiply
datapaths) would need separate design + verification work.
